...
func TestTerraformAwsRoute53Zone(t *testing.T) {
	...
	for _, b := range listResourceRecordSets.ResourceRecordSets {
		if *b.Type == record1["type"] {
			...
			var actualRecordValues []string
			for _, a := range b.ResourceRecords {
				actualRecordValues = append(actualRecordValues, *a.Value)
			}
			less := func(a, b string) bool { return a < b }
			assert.True(t, cmp.Diff(record1["records"], actualRecordValues, cmpopts.SortSlices(less)) == "")
			...
			assert.EqualValues(t, record2["records"], actualRecordValues)
			assert.EqualValues(t, expectedRecordName, *b.Name)
